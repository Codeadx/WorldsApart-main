...
public class RayMarching : MonoBehaviour {
    ...
    void CreateScene () {
        List<Shapes> allShapes = new List<Shapes> (FindObjectsOfType<Shapes> ());
        allShapes.Sort ((a, b) => a.operation.CompareTo (b.operation));

        List<Shapes> orderedShapes = new List<Shapes> ();
        ...
        for (int i = 0; i < allShapes.Count; i++) {
            ...
            if (allShapes[i].transform.parent == null) {
                ...
                for (int j = 0; j < parentShape.childCount; j++) {
                    if (parentShape.GetChild (j).GetComponent<Shapes> () != null) {
                        orderedShapes.Add (parentShape.GetChild (j).GetComponent<Shapes> ());
